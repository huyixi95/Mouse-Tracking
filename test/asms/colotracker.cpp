...
{
    im1 = cv::Mat(img.size(), CV_8UC1);
    im2 = cv::Mat(img.size(), CV_8UC1);
    im3 = cv::Mat(img.size(), CV_8UC1);
...
cv::Rect ColorTracker::track(cv::Mat & img, double x1, double y1, double x2, double y2) {
	...
	if (std::abs(std::log(scale)) > 0.05) {
        ...
        if (std::abs(std::log(scale*scaleB)) > 0.1){
			double alfha = 0.1*(defaultWidth / (float)(x2 - x1));
			width = (0.9 - alfha)*(x2 - x1) + 0.1*(x2 - x1)*scale + alfha*defaultWidth;
			height = (0.9 - alfha)*(y2 - y1) + 0.1*(y2 - y1)*scale + alfha*defaultHeight;
        }
    }
    ...
    return cv::Rect(modeCenter.x - width / 2, modeCenter.y - height / 2, width, height);
}
...
void ColorTracker::extractForegroundHistogram(int x1, int y1, int x2, int y2, Histogram & hist)
{
	...
	double wh_i = 1.0 / (w2*1.4142135623731 + 1);
	double hh_i = 1.0 / (h2*1.4142135623731 + 1);
